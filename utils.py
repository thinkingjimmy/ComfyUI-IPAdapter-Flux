...
def FluxUpdateModules(flux_model, ip_attn_procs, image_emb, is_patched):
    ...
    for i in range(dsb_count):
        if not is_patched:
            ...
            flux_model.diffusion_model.double_blocks[i]=DoubleStreamBlockIPA(flux_model.diffusion_model.double_blocks[i],ip_attn_procs[f"double_blocks.{i}"],image_emb)
        else:
    ...
    for i in range(ssb_count):
        if not is_patched:
            # initial ipa models with image embeddings
            flux_model.diffusion_model.single_blocks[i]=SingleStreamBlockIPA(flux_model.diffusion_model.single_blocks[i],ip_attn_procs[f"single_blocks.{i}"],image_emb)
        else:
            # replace image embeddings with new embeddings
